...
<div class="container-fluid mt-4">
    <div class="card p-4 shadow-sm">
        ...
        <form id="editDataDokter" method="POST" action="{{ route('admin.data_dokter.update', ['id' => $dokter->idDokter]) }}">
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="editNamaDokter" name="namaDokter" value="{{ old('namaDokter', $dokter->namaDokter) }}" required>
            </div>

            <div class="mb-3">
                <label for="editSpesialis" class="form-label">Spesialis</label>
                <input type="text" class="form-control" id="editSpesialis" name="spesialis" value="{{ old('spesialis', $dokter->spesialis) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="editHariPraktek" name="hariPraktek" required>
                    ...
                    @foreach($hari as $h)
                        <option value="{{ $h->idHari }}" {{ old('hariPraktek', $dokter->jadwaldokters->first()->hari->idHari ?? '') == $h->idHari ? 'selected' : '' }}>
                            {{ $h->namaHari }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="editJamPraktek" name="jamPraktek" required>
                    ...
                    @foreach($waktu as $w)
                        ...
                        <option value="{{ $w->idWaktu }}" {{ old('jamPraktek', $dokter->jadwaldokters->first()->waktu->idWaktu ?? '') == $w->idWaktu ? 'selected' : '' }}>
                            {{ $jam }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="editTglLahir" name="tglLahir" value="{{ old('tglLahir', $dokter->tglLahir) }}" required>
            </div>

            <div class="mb-3">
                <label for="editAlamatDokter" class="form-label">Alamat Dokter</label>
                <input type="text" class="form-control" id="editAlamatDokter" name="alamatDokter" value="{{ old('alamatDokter', $dokter->alamatDokter) }}" required>
            </div>

            <div class="mb-3">
                <label for="editNoTelepon" class="form-label">No Telepon</label>
                <input type="text" class="form-control" id="editNoTelepon" name="noTelepon" value="{{ old('noTelepon', $dokter->noTelepon) }}">
            </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll(".btn-warning").forEach(button => {
        button.addEventListener("click", function(event) {
            ...
            document.getElementById("editNamaDokter").value = row.querySelector("td:nth-child(2)").innerText.trim();
            document.getElementById("editSpesialis").value = row.querySelector("td:nth-child(3)").innerText.trim();
            document.getElementById("editTglLahir").value = row.querySelector("td:nth-child(5)").innerText.trim();
            document.getElementById("editAlamatDokter").value = row.querySelector("td:nth-child(6)").innerText.trim();
            document.getElementById("editNoTelepon").value = row.querySelector("td:nth-child(7)").innerText.trim();
            document.getElementById("editHariPraktek").value = this.getAttribute("data-hari");
            ...
            // Show modal
            var editModal = new bootstrap.Modal(document.getElementById('editDataDokterModal'));
            editModal.show();
        });
    });
});
